#telegram channel @wolfs2support
...
from wolf import CMD_HELP,  client
from wolf.wolfbot.heroku_var import config as Config
from wolf.wolfbot.heroku_var import config as Var
from wolf.wolfbot.heroku_var import config as var
from wolf.wolfbot.heroku_var import config
from telethon import Button, custom, events

from wolf import CMD_LIST
from wolf import tebot as tgbot
...
try:
  from wolf import tebot
except:
   tebot = None
   pass
from math import ceil
import asyncio
from wolf import CMD_HELP, BOTLOG, BOTLOG_CHATID,client
import os
from wolf.events import mrsammy, bot, itzjass
from wolf.util import admin_cmd
from wolf import tebot as tgbot
from wolf import bot as borg
wolfs = client = bot
from telethon import events
...
from wolf import CMD_LIST, CMD_HELP
import io

from wolf.utils import remove_plugin,load_module
...
if IN:
  @mrsammy(outgoing=True, pattern="^!help(?: |$|\n)([\s\S]*)")
  async def ban(event):
    if not BT:
...
if tebot:
 ...
 async def on_plug_in_callback_query_handler(event):
        ...
        if os.path.exists("./wolf/secrets.txt"):
            jsondata = json.load(open("./wolf/secrets.txt"))
            try:
 ...
 async def handler(event):
  try:
    ...
    if et == "back":
        ...
        return await event.edit ("Support Group For Report bugs & help @WolfBotHelp", buttons=tbu, link_preview=False)   
    if et in CMD_HELP: 
...
if tebot:
 ...
 async def inline_handler(event):
  ...
  if event.query.user_id == me.id and match:
            ...
            builder = event.builder
            secret = os.path.join("./wolf", "secrets.txt")
            try:
  ...
  if query.startswith("helpme"):
      ...
      if sad3:
	       tbu += [[ Button.inline(f"{sad3}"  , f"{sad3}")]]   
      result = builder.article("Help menu", text = "Support Group For Report bugs & help @WolfBotHelp", buttons=tbu, link_preview=False)      
...
#some codes taken form Cat bot Fixing them to wolfs was a task #Shivam
